...
Дипломный проект курса «Go-разработчик» Яндекс Практикума: планировщик задач.
...
Скопируйте `.env.example` в `.env` и задайте `TODO_PASSWORD`. Файл `.env` и база `scheduler.db` в git не коммитятся.

- `TODO_PORT` — порт HTTP-сервера (по умолчанию 7540)
- `TODO_DBFILE` — путь к SQLite-файлу
- `NUMNER_OF_OUTPUT_TASKS` — сколько задач на страницу
- `TODO_PASSWORD` — пароль для входа в веб-интерфейс
...
Перед использованием нужно авторизоваться на http://localhost:7540/login.html паролем из `TODO_PASSWORD`.
